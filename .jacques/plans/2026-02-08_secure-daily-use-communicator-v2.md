...
Four phases — security first, then UX, then features, then production hardening (including PWA).

**Existing codebase key files:**
- `src/main.ts` — bootstrap, ValidationPipe, static assets from `src/public`
- `src/app.module.ts` — root module, TypeORM config with synchronize:true
- `src/auth/` — auth.module, auth.controller, auth.service, jwt.strategy, jwt-auth.guard, DTOs
- `src/users/` — user.entity (id, email, password), users.service, users.module (no controller)
- `src/conversations/` — conversation.entity (userOne, userTwo), conversations.service, module
- `src/messages/` — message.entity (content, sender, conversation, createdAt), messages.service, module
- `src/chat/` — chat.gateway (WebSocket, Socket.IO, JWT via query+auth), chat.module
- `src/public/index.html` — full SPA client (RPG-themed, Press Start 2P font, login/register/chat)
- `docker-compose.yml`, `Dockerfile`, `package.json`, `tsconfig.json`

**Current client connects with:** `io(origin, { auth: { token }, query: { token } })`
...
- Modify `src/public/index.html` — auto-refresh access token on timer, handle 401 with refresh, store refresh token in memory
...
- Modify `src/public/index.html` — connect with `auth: { token }` only (remove `query: { token }`)
...
- Modify `src/chat/chat.gateway.ts` — broadcast `userOnline`/`userOffline` events, track `Map<userId, Set<socketId>>`, add `getOnlineUsers` handler, include `isOnline` in conversation list
...
- Modify `src/public/index.html` — debounce 300ms typing emission, "User is typing..." display

### 2.4 Read receipts
- Modify `src/messages/message.entity.ts` — add `readAt: Date | null` column
- Modify `src/messages/messages.service.ts` — add `markAsRead(conversationId, userId)`
- Modify `src/chat/chat.gateway.ts` — add `markRead` handler, emit `messagesRead`
- Modify `src/public/index.html` — emit markRead on open, show checkmarks
...
- Modify `src/messages/messages.service.ts` — accept `{ before, limit }`, return `{ messages, hasMore }`
...
- Modify `src/public/index.html` — media queries for <768px, full-width sidebar with back button, fluid layout, larger touch targets (min 44px)
...
- Modify `src/main.ts` — serve `/uploads` as static
- Modify `src/public/index.html` — avatar circles, initials fallback

### 3.4 Last seen timestamps
- Modify `src/users/user.entity.ts` — add `lastSeenAt` column
- Modify `src/chat/chat.gateway.ts` — update lastSeen on disconnect
...
### 3.7 Emoji support
- Modify `src/public/index.html` — emoji picker popup, font-family fallback
...
### 4.6 PWA (Progressive Web App) — installable on phones
- Create `src/public/manifest.json`:
  - `name`: "RPG Chat", `short_name`: "RPGChat", `start_url`: "/", `display`: "standalone"
  - `background_color`: "#0a0a2e", `theme_color`: "#4a4ae0"
  - Icons: 192x192 and 512x512 PNG (generate from app icon)
- Create `src/public/sw.js` — Service Worker:
  - Cache static assets (index.html, fonts, socket.io client)
  - Network-first strategy for API calls
  - Offline fallback page
- Modify `src/public/index.html`:
  - Add `<link rel="manifest" href="/manifest.json">`
  - Add `<meta name="theme-color" content="#4a4ae0">`
  - Add `<meta name="apple-mobile-web-app-capable" content="yes">`
  - Add `<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">`
  - Add `<link rel="apple-touch-icon" href="/icons/icon-192.png">`
  - Register service worker in JS: `navigator.serviceWorker.register('/sw.js')`
- Create `src/public/icons/` — generate 192x192 and 512x512 PNG icons (RPG sword theme)
- Modify `src/main.ts` — ensure static assets served with correct MIME types
- **Note**: Requires HTTPS in production for Service Worker to work. Localhost is exempt for dev.
...
4. Phase-specific checks:
   ...
   - **Phase 4**: `GET /health` returns ok, `/api/docs` shows Swagger UI, migrations run on startup, PWA install prompt appears in Chrome on phone, app works from home screen in standalone mode
