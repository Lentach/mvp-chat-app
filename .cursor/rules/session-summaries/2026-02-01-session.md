...
### 9. Avatar update fix + gallery direct (2026-02-01)
- **Problem:** Po wyborze nowego zdjęcia w Settings avatar się nie zmieniał (odświeżał się ale zostawał ten sam).
- **Przyczyna:** Backend po uploadzie wywoływał `deleteAvatar(oldPublicId)` — Cloudinary używa tego samego `public_id` przy overwrite, więc usuwaliśmy dopiero wgrany obraz.
- **Fix:** `updateProfilePicture` usuwa stary avatar tylko gdy `oldPublicId !== newPublicId`.
- **UI:** Ikona kamery otwiera galerię od razu (bez dialogu Take photo / Choose from gallery). Usunięto `ProfilePictureDialog`.
- **Pliki:** backend users.service.ts, frontend settings_screen.dart; usunięto profile_picture_dialog.dart.

### 10. Code review + redesign CLAUDE.md (2026-02-01)
- **Code review:** Przegląd uncommitted zmian (Delete Account Cascade Fix). Ocena: gotowe do merge z poprawką — rekomendacja transakcji dla atomowości. Brak błędów krytycznych.
- **CLAUDE.md redesign:** Przeprojektowano pod kątem czytelności dla agenta. Skrócono z ~660 do ~160 linii. Kluczowe zasady na początku (tabela), Quick Reference w sekcji 3, usunięto redundancje (Recent Changes duplikowały Bug History), zwarta Bug Fix History (tylko lekcje). Nowa struktura: Critical → Quick Start → Quick Reference → Architecture → APIs → Gotchas → Recent → Env → Bug History → Limitations → Tech Debt.

## Stan projektu
Avatar użytkowników przechowywany w Cloudinary. Działa tryb jasny/ciemny (Theme). Light mode: neutralna paleta (fiolet #4A154B). Dark mode: paleta Delete Account (akcent #FF6666, borderDark, mutedDark), kafelki w Settings w stylu komunikatu ostrzeżenia. Aplikacja wymaga poprawnego `.env` z danymi Cloudinary. Zmiana avatara działa poprawnie; ikona kamery otwiera galerię bezpośrednio. CLAUDE.md przeprojektowany — źródło prawdy dla agentów.
